...
function countdownTimer() {
  ...
  let counterOfHour = Math.floor(
    (timeStep -
      Math.floor(timeStep / millisecondConverter) * millisecondConverter) /
      3600000
  );
  hour.innerHTML = counterOfHour;
  let counterOfMinute = Math.floor(
    (timeStep -
      (Math.floor(timeStep / millisecondConverter) * millisecondConverter +
        counterOfHour * 3600000)) /
      60000
  );
  minute.innerHTML = counterOfMinute;
  let counterOfSecond = Math.floor(
    (timeStep -
      (Math.floor(timeStep / millisecondConverter) * millisecondConverter +
        counterOfHour * 3600000 +
        counterOfMinute * 60000)) /
      1000
  );
